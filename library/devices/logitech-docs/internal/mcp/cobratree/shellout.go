...
func shellOutToCLI(cliPath func() (string, error), commandPath []string, blockedStructuredArgs map[string]bool, allowedStructuredArgs map[string]bool, positionals []positionalArg, readOnly bool, positionalWriteSinks map[int]bool) server.ToolHandlerFunc {
	...
	return func(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
		if lookupErr != nil {
			return boundedToolResultError(fmt.Sprintf("companion CLI binary not found: %v\nTried sibling lookup, LOGITECH_DOCS_CLI_PATH env var, and PATH.", lookupErr)), nil
		}
...
// malicious config file, change the delivery target, or turn on receipt and
// audit writing at a caller-selected host path, all of which sit outside the
...
// readOnlyWriteSinkFlags are flags whose value is a host filesystem
// destination. A tool annotated mcp:read-only must not accept them: the
// annotation tells the client the call cannot mutate anything, and a
// caller-selected path breaks that promise. This is the flag analogue of the
// positionalWriteSinks guard above. Commands that are not read-only still get
// these flags, because writing is what they are for.
var readOnlyWriteSinkFlags = map[string]bool{
	"db":   true,
	"save": true,
}
