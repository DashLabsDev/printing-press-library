...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LOGITECH_DOCS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LOGITECH_DOCS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "logitech-docs-pp-cli/feedback")
...
When ` + "`LOGITECH_DOCS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LOGITECH_DOCS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  logitech-docs-pp-cli feedback list
  logitech-docs-pp-cli feedback list --limit 5
  logitech-docs-pp-cli feedback list --json`,
