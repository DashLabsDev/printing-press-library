...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(w, "  Settings -> API Access -> accept terms -> Generate API Key. The key is per-user and covers every verified site.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export BING_WEBMASTER_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  bing-webmaster-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintf(w, "  bing-webmaster-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bing-webmaster-pp-cli auth set-token YOUR_TOKEN_HERE",
