...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bing-webmaster-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BING_WEBMASTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BING_WEBMASTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bing-webmaster-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bing-webmaster-pp-cli/feedback.jsonl.
When ` + "`BING_WEBMASTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BING_WEBMASTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bing-webmaster-pp-cli feedback list
  bing-webmaster-pp-cli feedback list --limit 5
  bing-webmaster-pp-cli feedback list --json`,
