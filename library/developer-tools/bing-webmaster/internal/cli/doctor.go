...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bing-webmaster-pp-cli doctor
  bing-webmaster-pp-cli doctor --json
  bing-webmaster-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BING_WEBMASTER_API_KEY=<your-key>"
					report["auth_key_url"] = "https://www.bing.com/webmasters"
					report["auth_instructions"] = "Settings -> API Access -> accept terms -> Generate API Key. The key is per-user and covers every verified site."
				} else {
					authConfigured = true
			...
			if os.Getenv("BING_WEBMASTER_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "BING_WEBMASTER_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BING_WEBMASTER_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'bing-webmaster-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'bing-webmaster-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'bing-webmaster-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'bing-webmaster-pp-cli sync' to refresh."
	}
	return report
}
