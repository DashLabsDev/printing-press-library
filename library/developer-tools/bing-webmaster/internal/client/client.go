...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "bing-webmaster-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Bing-specific: rewrite Microsoft /Date(...)/ literals and strip the
		// WCF {"d": ...} envelope so every command and the store see clean data.
		respBody = normalizeMSDates(respBody)
		respBody = unwrapBingEnvelope(respBody)
