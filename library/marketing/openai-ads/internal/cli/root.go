...
type rootFlags struct {
	...
	// invocation. Mirrors the OPENAI_ADS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "openai-ads-pp-cli",
		Short: `Openai Ads CLI — The first OpenAI Ads client that can write, and the only one that keeps local history.`,
		Long: `Openai Ads CLI — The first OpenAI Ads client that can write, and the only one that keeps local history.
...
Health check: run 'openai-ads-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("openai-ads-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'openai-ads-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAdAccountCmd(flags))
	rootCmd.AddCommand(newAdGroupsCmd(flags))
	rootCmd.AddCommand(newAdsCmd(flags))
	rootCmd.AddCommand(newCampaignsCmd(flags))
	rootCmd.AddCommand(newConversionsCmd(flags))
	rootCmd.AddCommand(newCustomAudiencesCmd(flags))
	...
	rootCmd.AddCommand(newGeoLookupPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBidCheckCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDriftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFatigueCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelGeoCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOrphansCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPaceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReviewWatchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTreeCmd(flags))
