...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/openai-ads/internal/learn/lookups"
...
Resource scoping:
  ...
  openai-ads-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openai-ads-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openai-ads-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openai-ads-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openai-ads-pp-cli sync --latest-only`,
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "ad-groups/ad_groups_insights":
		return paginationDefaults{
			cursorParam: "after",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "campaigns/campaigns_insights":
		return paginationDefaults{
			cursorParam: "after",
			cursorType:  "cursor",
			limitParam:  "limit",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "ad-account":
		return db.UpsertAdAccount(data)
	case "ad-groups":
		return db.UpsertAdGroups(data)
	case "ad_groups_activate":
		return db.UpsertAdGroupsActivate(data)
	case "ad_groups_archive":
		return db.UpsertAdGroupsArchive(data)
	case "ad_groups_insights":
		return db.UpsertAdGroupsInsights(data)
	case "ad_groups_pause":
		return db.UpsertAdGroupsPause(data)
	case "ads":
		return db.UpsertAds(data)
	case "ads_activate":
		return db.UpsertAdsActivate(data)
	case "ads_archive":
		return db.UpsertAdsArchive(data)
	case "ads_insights":
		return db.UpsertAdsInsights(data)
	case "ads_pause":
		return db.UpsertAdsPause(data)
	case "campaigns":
		return db.UpsertCampaigns(data)
	case "campaigns_activate":
		return db.UpsertCampaignsActivate(data)
	case "campaigns_archive":
		return db.UpsertCampaignsArchive(data)
	case "campaigns_insights":
		return db.UpsertCampaignsInsights(data)
	case "campaigns_pause":
		return db.UpsertCampaignsPause(data)
	case "conversions":
		return db.UpsertConversions(data)
	case "custom-audiences":
		return db.UpsertCustomAudiences(data)
	case "custom_audiences_archive":
		return db.UpsertCustomAudiencesArchive(data)
	case "geo-lookup":
		return db.UpsertGeoLookup(data)
	case "upload":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"ad-account":       "/ad_account/insights",
		"ad-groups":        "/ad_groups",
		"ads":              "/ads",
		"campaigns":        "/campaigns",
		"conversions":      "/conversions/event_settings",
...
type dependentResourceDef struct {
	...
	// ParentQueryParam scopes a dependent whose parent link is a QUERY
	// parameter rather than a path segment (e.g. GET /ad_groups?campaign_id=...).
	// When set, PathTemplate stays flat and the parent id is injected into the
	// request params instead of substituted into the URL path.
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		// ad groups are parent-keyed by campaign: GET /ad_groups requires the
		// campaign_id query param (TASK 0). Iterate campaigns and persist each
		// ad group's campaign_id so the hierarchy can be reconstructed.
		{Name: "ad-groups", ParentTable: "campaigns", ParentIDParam: "campaign_id", ParentQueryParam: "campaign_id", PathTemplate: "/ad_groups", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.campaigns_id", PathParams: []dependentPathParamDef{
			{Param: "campaign_id", Field: "id"},
		}},
		{Name: "ad_groups_insights", ParentTable: "ad-groups", ParentIDParam: "ad_group_id", PathTemplate: "/ad_groups/{ad_group_id}/insights", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.ad-group", PathParams: []dependentPathParamDef{
			{Param: "ad_group_id", Field: "id"},
		}},
		// ads are parent-keyed by ad group (TASK 0).
		{Name: "ads", ParentTable: "ad-groups", ParentIDParam: "ad_group_id", ParentQueryParam: "ad_group_id", PathTemplate: "/ads", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.ad-groups_id", PathParams: []dependentPathParamDef{
			{Param: "ad_group_id", Field: "id"},
		}},
		{Name: "ads_insights", ParentTable: "ads", ParentIDParam: "ad_id", PathTemplate: "/ads/{ad_id}/insights", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.ad", PathParams: []dependentPathParamDef{
			{Param: "ad_id", Field: "id"},
		}},
		{Name: "campaigns_insights", ParentTable: "campaigns", ParentIDParam: "campaign_id", PathTemplate: "/campaigns/{campaign_id}/insights", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.campaign", PathParams: []dependentPathParamDef{
			{Param: "campaign_id", Field: "id"},
		}},
	}
}
...
func syncOneParent(
	...
	for {
		...
		// Query-param-scoped dependents carry the parent id as a request
		// parameter instead of a path segment. Set before user flags so an
		// explicit --resource-param can still override it deliberately.
		if dep.ParentQueryParam != "" {
			params[dep.ParentQueryParam] = store.BareResourceID(parentID)
		}
...
var reconcileTypedTables = map[string]string{
	...
	"ad-groups":                "ad_groups",
	"ad_groups_activate":       "ad_groups_activate",
	"ad_groups_archive":        "ad_groups_archive",
	"ad_groups_insights":       "ad_groups_insights",
	"ad_groups_pause":          "ad_groups_pause",
	"ads":                      "ads",
	...
	"ads_pause":                "ads_pause",
	"campaigns":                "campaigns",
	"campaigns_activate":       "campaigns_activate",
	"campaigns_archive":        "campaigns_archive",
	"campaigns_insights":       "campaigns_insights",
	"campaigns_pause":          "campaigns_pause",
	"conversions":              "conversions",
	"custom-audiences":         "custom_audiences",
	"custom_audiences_archive": "custom_audiences_archive",
	"geo-lookup":               "geo_lookup",
	"upload":                   "upload",
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "ad-account\x00/ad_account/insights":
		return []string{"data"}
	case "ad-groups\x00/ad_groups":
		return []string{"context_hints"}
	case "ad-groups\x00/ad_groups/{ad_group_id}":
		return []string{"context_hints"}
	case "ads\x00/ads":
		return []string{"data"}
	case "campaigns\x00/campaigns":
		return []string{"conversion_event_setting_ids"}
	case "campaigns\x00/campaigns/{campaign_id}":
		return []string{"conversion_event_setting_ids"}
	case "conversions\x00/conversions/event_settings":
		return []string{"data"}
	case "conversions\x00/conversions/insights":
		return []string{"data"}
	case "custom-audiences\x00/custom_audiences":
		return []string{"data"}
	case "geo-lookup\x00/geo_lookup/search":
