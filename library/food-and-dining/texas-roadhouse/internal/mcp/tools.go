...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/cli"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/client"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/config"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/mcp/bound"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/mcp/cobratree"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/platform"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/internal/store"
)
...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("mapbox_get_mapbox.places",
			mcplib.WithDescription("GET /api/mapbox/geocoding/v5/mapbox.places/{id}. Required: id. Optional: country, types. Returns array of MapboxPlaceFeature."),
			mcplib.WithString("country", mcplib.Description("")),
		...
		makeAPIHandler("GET", "/api/mapbox/geocoding/v5/mapbox.places/{id}", true, false, nil, mcpPageConfig{}, []mcpParamBinding{{PublicName: "country", WireName: "country", Location: "query"}, {PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "types", WireName: "types", Location: "query"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("stores_list_near",
			mcplib.WithDescription("GET /api/stores/near. Optional: lat, limit, long (plus 5 more). Returns array of NearRestaurant."),
			mcplib.WithString("lat", mcplib.Description("")),
			mcplib.WithNumber("limit", mcplib.Description("")),
			mcplib.WithString("long", mcplib.Description("")),
			mcplib.WithString("nomnom", mcplib.Description("")),
			mcplib.WithNumber("nomnom_calendars_from", mcplib.Description("")),
			mcplib.WithNumber("nomnom_calendars_to", mcplib.Description("")),
			mcplib.WithString("nomnom_exclude_extref", mcplib.Description("")),
			mcplib.WithNumber("radius", mcplib.Description("")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/api/stores/near", true, false, nil, mcpPageConfig{}, []mcpParamBinding{{PublicName: "lat", WireName: "lat", Location: "query"}, {PublicName: "limit", WireName: "limit", Location: "query"}, {PublicName: "long", WireName: "long", Location: "query"}, {PublicName: "nomnom", WireName: "nomnom", Location: "query"}, {PublicName: "nomnom_calendars_from", WireName: "nomnom_calendars_from", Location: "query"}, {PublicName: "nomnom_calendars_to", WireName: "nomnom_calendars_to", Location: "query"}, {PublicName: "nomnom_exclude_extref", WireName: "nomnom_exclude_extref", Location: "query"}, {PublicName: "radius", WireName: "radius", Location: "query"}}, []string{}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("texasroadhouse_get_quote",
			mcplib.WithDescription("GET /api/texasroadhouse/waitlist/{waitlist_id}/quote. Required: waitlist_id. Returns the Quote."),
			mcplib.WithString("waitlist_id", mcplib.Required(), mcplib.Description("The waitlist_id path segment")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/api/texasroadhouse/waitlist/{waitlist_id}/quote", true, false, nil, mcpPageConfig{}, []mcpParamBinding{{PublicName: "waitlist_id", WireName: "waitlist_id", Location: "path"}}, []string{"waitlist_id"}),
	)
	s.AddTool(
		mcplib.NewTool("texasroadhouse_get_settings",
			mcplib.WithDescription("GET /api/texasroadhouse/waitlist/{waitlist_id}/settings. Required: waitlist_id. Returns array of SettingSpecialSeatingType."),
			mcplib.WithString("waitlist_id", mcplib.Required(), mcplib.Description("The waitlist_id path segment")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/api/texasroadhouse/waitlist/{waitlist_id}/settings", true, false, nil, mcpPageConfig{}, []mcpParamBinding{{PublicName: "waitlist_id", WireName: "waitlist_id", Location: "path"}}, []string{"waitlist_id"}),
	)
	s.AddTool(
		mcplib.NewTool("texasroadhouse_get_status",
			mcplib.WithDescription("GET waitlist request status. Query clientid=texasroadhouse. Required: waitlist_id, request_id. Optional: clientid. Returns the WaitlistStatus."),
			mcplib.WithString("waitlist_id", mcplib.Required(), mcplib.Description("Store extref (not internal store id). Springfield MO is 218.")),
			mcplib.WithString("request_id", mcplib.Required(), mcplib.Description("Waitlist request id from a successful submit.")),
			mcplib.WithString("clientid", mcplib.Description("Must be texasroadhouse")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/api/texasroadhouse/waitlist/{waitlist_id}/requests/{request_id}/status", true, false, nil, mcpPageConfig{}, []mcpParamBinding{{PublicName: "waitlist_id", WireName: "waitlist_id", Location: "path"}, {PublicName: "request_id", WireName: "request_id", Location: "path"}, {PublicName: "clientid", WireName: "clientid", Location: "query"}}, []string{"waitlist_id", "request_id"}),
	)
	s.AddTool(
		mcplib.NewTool("texasroadhouse_get_test",
			mcplib.WithDescription("GET /api/texasroadhouse/waitlist/{waitlist_id}/test. Required: waitlist_id. Returns the Test."),
			mcplib.WithString("waitlist_id", mcplib.Required(), mcplib.Description("The waitlist_id path segment")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
	...
	// transcripts or logs, while the CLI can receive guest identity from stdin
	// or an interactive prompt without making it part of a tool schema.
	...
	s.AddTool(
		mcplib.NewTool("sql",
			...
			mcplib.WithString("query", mcplib.Required(), mcplib.Description("SQL query (SELECT or WITH...SELECT). Synced records live in resources(resource_type, id, data); filter by resource_type and use json_extract on data, e.g. SELECT json_extract(data,'$.name') FROM resources WHERE resource_type='mapbox'.")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
		),
		handleSQL,
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run texas-roadhouse-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run texas-roadhouse-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run texas-roadhouse-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func mcpSQLQueryError(err error) string {
	...
	if strings.Contains(strings.ToLower(msg), "no such table") {
		return fmt.Sprintf("query failed: %v. Synced records live in resources(resource_type, id, data), not one SQL table per resource. Filter by resource_type, for example resource_type='mapbox', and read JSON fields with json_extract(data,'$.field').", err)
	}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		...
		"resources": []map[string]any{
			{
				"name":        "mapbox",
				"description": "Operations on mapbox.places",
				"endpoints":   []string{"get_mapbox.places"},
				"searchable":  true,
			},
			{
				"name":        "stores",
				"description": "Operations on near",
				"endpoints":   []string{"list_near"},
				"syncable":    true,
				"searchable":  true,
			},
			{
