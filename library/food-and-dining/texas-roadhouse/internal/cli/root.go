...
type rootFlags struct {
	...
	// invocation. Mirrors the TEXAS_ROADHOUSE_NO_LEARN env var.
	noLearn bool
...
Run 'texas-roadhouse-pp-cli doctor' to verify connectivity.`,
	...
	rootCmd.SetVersionTemplate("texas-roadhouse-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select bbox,center,context)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'texas-roadhouse-pp-cli profile list')")
	...
	rootCmd.AddCommand(newTexasroadhouseCmd(flags))
	...
	rootCmd.AddCommand(newMapboxPromotedCmd(flags))
